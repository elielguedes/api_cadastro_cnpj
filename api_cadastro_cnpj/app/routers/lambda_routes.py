...
# Autenticação simplificada para Lambda routes
security = HTTPBearer()

async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Usuario:
    """
    Dependency simples para obter usuário autenticado
    """
    ...
    # Mock user para demonstração - substituir por validação JWT real
    return Usuario(
        id=1,
